...
export class Screen6 extends Component {
  render() {
    return <BaseScreen number={6}>
      ...
      <CT.Row padding={20}>
        <CT.Head1 color='white' numberOfLines={2} fontSize={42}>{"Out of \nthe Wild Ocean"}</CT.Head1>
        <CT.Row>
          <CT.Stars value={4.5} size={18} />
        </CT.Row>
        <CT.RowLine height={100} justifyContent='space-evenly'>
...
const Info = (props) => {
  return <CT.Div alignItems='center'>
    <CT.Head4 color='white' style={{textAlign: 'center'}}>{props.text}</CT.Head4>
    <CT.IconLabel padding={1}
      icon={<CT.Icon name={props.icon} color='white' size={12} {...props}/>}
      label={<CT.Label theme='Head8' color='white'>{props.label}</CT.Label>}
    />
  </CT.Div>
}
